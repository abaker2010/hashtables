...
{
	unint index = inputHash;
	openBucket emptyBucket;
	...
	if (search != openAddressTable.end())
	{
		   ...
		   // openBucket* Pointer = &openAddressTable[index];
			//Pointer->collision = true;
			openAddressTable[index].collision = true;
			openCollisions++;
			index++;
			addToOpenAddress(index, text);	
			return;
	}
		
	emptyBucket.collision = false;
	emptyBucket.entry_=text;
	openAddressTable[index] = emptyBucket;
}
...
{
	unint index = inputHash;
	chainBucket emptyBucket = {false, new List() };
	...
	auto search = chainingTable.find(inputHash);
		...
		if (search != chainingTable.end())
		{
			...
			chainingTable[index].collision = true;
			chainedCollisions++;
			chainingTable[index].bucketList->insertAsLast(text);
			
			return;
		}

	emptyBucket.collision = false;
	emptyBucket.bucketList->insertAsLast(text);
	chainingTable[index] = emptyBucket;
}
